...
\begin{table}[H]\caption{Comparing the results for importance/brute force sampling. Energy are in atomic units (a.u.). Here the parameter $\alpha$ is set to 0.9 and number of MC cycles are 2$^{21}$. Acc. is short for acceptance and is here given in \%, $\sigma_B$ is the standard deviation after resampling with the blocking method, SEM is the standard deviation of the mean and $t_{CPU}$ is the CPU time used by the program in units of seconds. }\label{tab:importance_no_interaction_2p}
\begin{tabular}{l|l|l} 
